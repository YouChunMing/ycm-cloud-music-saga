...
describe('banner root saga', () => {
    it('yeild fork getHomeData', () => {
        const gen = saga();
        expect(gen.next().value).toEqual(takeLatest(Banner_Redux.types.GET_HOME_DATA_REQUEST, getHomeData));
...
describe('banner getHomeData saga', () => {
    ...
    it('if getHomeData Api success, yeild put getHomeDataSuccess action', () => {
        ...
        expect(clone.next(response).value).toEqual(put(Banner_Redux.actions.getHomeDataSuccess(response['data'])));
    ...
    it('if getHomeData Api failure, yeild put getHomeDataSuccess action', () => {
        const clone = gen.clone();
        const error = {};
        expect(clone.throw(error).value).toEqual(put(Banner_Redux.actions.getHomeDataFailure(error)));
        expect(clone.next().done).toEqual(true);
    });
